...
1.  **NO Backends / Databases:** Do not suggest Supabase, Firebase, Neon, or SQL. We use **LocalStorage** only.
2.  **NO Authentication:** No login screens. No Clerk, No Auth0.
3.  **NO State Management Libraries:** Do not install Redux, Zustand, or MobX. Use React `useState` or React Flow's internal state only.
4.  **NO Complex TypeScript:** If a type error is hard to fix, use `any` or `ts-ignore`. We are hacking, not engineering.
5.  **NO Tests:** Do not write unit tests or integration tests.
6.  **NO Form Libraries:** Do not use React Hook Form or Zod. Use standard HTML `<input>` and `onChange`.
...
2.  **Does it require a server?** (Yes = **KILL IT**)
3.  **Does it require learning a new library?** (Yes = **KILL IT**)
4.  **Can we fake it with CSS?** (Yes = Do that instead of logic)
...
*If the code breaks and we are stuck for more than 15 minutes:*
