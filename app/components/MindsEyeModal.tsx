...
export function MindsEyeModal(props: {
  open: boolean;
  onClose: () => void;
  imageUrl: string | null;
  onApplyPlan: (plan: MindsEyePlan) => void;
}) {
  const { open, onClose, imageUrl, onApplyPlan } = props;
  ...
  async function handleAnalyze() {
    ...
    try {
      ...
      const res = await fetch("/api/grimpy/analyze-sketch", {
        method: "POST",
              ...
              <div className="space-y-3">
                ...
                <div className="rounded-2xl border border-cyan-300/15 bg-black/30 p-2">
                  {imageUrl ? (
                    ...
                    <img
                      src={imageUrl}
                      alt="Mind’s Eye input"
                      className="max-h-[320px] w-full rounded-xl object-contain"
                    />
                  ) : (
                    <div className="p-6 text-center text-xs text-cyan-100/60">
                      No image selected.
                    </div>
                  )}
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  {error ? (
                    <div className="text-xs text-rose-200/90">{error}</div>
                  ) : null}
                </div>
              </div>
                ...
                <div className="rounded-2xl border border-cyan-300/15 bg-black/30 p-4">
                  {plan ? (
                    <div className="space-y-3">
                      ...
                      <div>
                        <div className="text-xs font-bold tracking-widest text-cyan-200/70">
                          SUMMARY
                        </div>
                        <div className="mt-1 text-sm text-cyan-50/90 whitespace-pre-wrap">
                          {plan.summary}
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <div className="text-[11px] text-cyan-100/60">
                          Creates 1 strategy + {plan.tactics?.length ?? 0} tactics
                        </div>
                      </div>
                    </div>
                  ) : (
                    <div className="text-xs text-cyan-100/60">
                      {loading ? "Waiting for Grimpy…" : "Run Analyze to generate a plan."}
                    </div>
                  )}
                </div>
              </div>
